...
export OPENMODELICAHOME="/c/OM116/${OM_ENCRYPT}OM${PLATFORM}/build"
export OPENMODELICALIBRARY="/c/OM116/${OM_ENCRYPT}OM${PLATFORM}/build/lib/omlibrary"

# have OMDEV in Msys version
export OMDEV=/c/OM116/OMDev/

# update OMDev
cd /c/OM116/OMDev/
git checkout OM116maintenance
git pull

# update OpenModelica
cd /c/OM116/${OM_ENCRYPT}OM${PLATFORM}
git checkout master
...
# Directory prefix
export OMC_INSTALL_PREFIX="/c/OM116/OpenModelica_releases/${OM_ENCRYPT}${REVISION}/"
...
# clean
rm -rf /c/OM116/OpenModelica_releases/${OM_ENCRYPT}/v*
rm -rf build
...
cd /c/OM116/${OM_ENCRYPT}OM${PLATFORM}/OMSetup
git checkout maintenance/v1.16
git pull
...
rm -rf OMPython
git clone https://github.com/OpenModelica/OMPython -q -b master /c/OM116/${OM_ENCRYPT}OM${PLATFORM}/OMPython
# build OMPython
...
git clone https://github.com/PySimulator/PySimulator -q -b master /c/OM116/${OM_ENCRYPT}OM${PLATFORM}/build/share/omc/scripts/PythonInterface/PySimulator

# get Figaro
cd /c/OM116/${OM_ENCRYPT}OM${PLATFORM}/build/share
...
mkdir -p public_html/omc/builds/windows/releases/1.16/maintenance/${OM_ENCRYPT}${PLATFORM}/older
cd public_html/omc/builds/windows/releases/1.16/maintenance/${OM_ENCRYPT}${PLATFORM}/
#rm -f older/*.* || true
mv -f OpenModelica* older/ || true
ENDSSH
scp OpenModelica*${PLATFORM}* ${SSHUSER}@build.openmodelica.org:public_html/omc/builds/windows/releases/1.16/maintenance/${OM_ENCRYPT}${PLATFORM}/
ssh ${SSHUSER}@build.openmodelica.org <<ENDSSH
#commands to run on remote host
cd public_html/omc/builds/windows/releases/1.16/maintenance/${OM_ENCRYPT}${PLATFORM}/
pwd
