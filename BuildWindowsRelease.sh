...
export OPENMODELICAHOME="c:/OM125/${OM_ENCRYPT}OM${PLATFORM}/build"
export OPENMODELICALIBRARY="c:/OM125/${OM_ENCRYPT}OM${PLATFORM}/build/lib/omlibrary"
...
# update OpenModelica
cd /c/OM125/${OM_ENCRYPT}OM${PLATFORM}
git checkout master
...
# Directory prefix
export OMC_INSTALL_PREFIX="/c/OM125/OpenModelica_releases/${OM_ENCRYPT}${REVISION}/"
# file suffix
...
# clean
rm -rf /c/OM125/OpenModelica_releases/${OM_ENCRYPT}/v*
rm -rf build
...
cd /c/OM125/${OM_ENCRYPT}OM${PLATFORM}/OMSetup
git checkout maintenance/v1.25
git pull
...
rm -rf OMPython
git clone https://github.com/OpenModelica/OMPython -q -b master /c/OM125/${OM_ENCRYPT}OM${PLATFORM}/OMPython
...
cd /c/OM125/${OM_ENCRYPT}OM${PLATFORM}/build/share/doc/omc
wget --no-check-certificate https://openmodelica.org/doc/openmodelica-doc-v1.25.0.tar.xz
tar -xJf openmodelica-doc-v1.25.0.tar.xz --strip-components=2
rm openmodelica-doc-v1.25.0.tar.xz
wget --no-check-certificate https://openmodelica.org/doc/OpenModelicaUsersGuide/OpenModelicaUsersGuide-1.25.pdf
cp OpenModelicaUsersGuide-1.25.pdf OpenModelicaUsersGuide-latest.pdf
...
git clone https://github.com/PySimulator/PySimulator -q -b master /c/OM125/${OM_ENCRYPT}OM${PLATFORM}/build/share/omc/scripts/PythonInterface/PySimulator

# get Figaro - let's not do that for now as it has log4j inside
#cd /c/OM125/${OM_ENCRYPT}OM${PLATFORM}/build/share
...
FILE_PATH=/var/www/build.openmodelica.org/omc/builds/windows/releases/1.25/maintenance/${OM_ENCRYPT}${PLATFORM}/${OMC_FILE_PREFIX}.exe
...
#commands to run on remote host
mkdir -p /var/www/build.openmodelica.org/omc/builds/windows/releases/1.25/maintenance/${OM_ENCRYPT}${PLATFORM}/older
cd /var/www/build.openmodelica.org/omc/builds/windows/releases/1.25/maintenance/${OM_ENCRYPT}${PLATFORM}/
#rm -f older/*.* || true
mv -f OpenModelica* older/ || true
ENDSSH
scp  -i $HOME/.ssh/id_rsa -o UserKnownHostsFile=$HOME/.ssh/known_hosts OpenModelica*${PLATFORM}* ${SSHUSER}@build.openmodelica.org:/var/www/build.openmodelica.org/omc/builds/windows/releases/1.25/maintenance/${OM_ENCRYPT}${PLATFORM}/
ssh  -i $HOME/.ssh/id_rsa -o UserKnownHostsFile=$HOME/.ssh/known_hosts ${SSHUSER}@build.openmodelica.org <<ENDSSH
#commands to run on remote host
cd /var/www/build.openmodelica.org/omc/builds/windows/releases/1.25/maintenance/${OM_ENCRYPT}${PLATFORM}/
pwd
