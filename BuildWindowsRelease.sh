...
export OPENMODELICAHOME="/c/OM117/${OM_ENCRYPT}OM${PLATFORM}/build"
export OPENMODELICALIBRARY="/c/OM117/${OM_ENCRYPT}OM${PLATFORM}/build/lib/omlibrary"

# have OMDEV in Msys version
export OMDEV=/c/OM117/OMDev/

# update OMDev
cd /c/OM117/OMDev/
git checkout OM117maintenance
git pull

# update OpenModelica
cd /c/OM117/${OM_ENCRYPT}OM${PLATFORM}
git checkout master
...
# Directory prefix
export OMC_INSTALL_PREFIX="/c/OM117/OpenModelica_releases/${OM_ENCRYPT}${REVISION}/"
...
# clean
rm -rf /c/OM117/OpenModelica_releases/${OM_ENCRYPT}/v*
rm -rf build
...
cd /c/OM117/${OM_ENCRYPT}OM${PLATFORM}/OMSetup
git checkout maintenance/v1.17
git pull
...
rm -rf OMPython
git clone https://github.com/OpenModelica/OMPython -q -b master /c/OM117/${OM_ENCRYPT}OM${PLATFORM}/OMPython
# build OMPython
...
cd /c/OM117/${OM_ENCRYPT}OM${PLATFORM}/build/share/doc/omc
wget --no-check-certificate https://openmodelica.org/doc/openmodelica-doc-v1.17.0.tar.xz
tar -xJf openmodelica-doc-v1.17.0.tar.xz --strip-components=2
rm openmodelica-doc-v1.17.0.tar.xz
wget --no-check-certificate https://openmodelica.org/doc/OpenModelicaUsersGuide/OpenModelicaUsersGuide-1.17.pdf
cp OpenModelicaUsersGuide-1.17.pdf OpenModelicaUsersGuide-latest.pdf
...
git clone https://github.com/PySimulator/PySimulator -q -b master /c/OM117/${OM_ENCRYPT}OM${PLATFORM}/build/share/omc/scripts/PythonInterface/PySimulator

# get Figaro
cd /c/OM117/${OM_ENCRYPT}OM${PLATFORM}/build/share
...
makensis //DPLATFORMVERSION="${PLATFORM::-3}" OpenModelicaSetup.nsi > trace.txt 2>&1 || cat trace.txt && exit 1
# move the installer
...
mkdir -p public_html/omc/builds/windows/releases/1.17/maintenance/${OM_ENCRYPT}${PLATFORM}/older
cd public_html/omc/builds/windows/releases/1.17/maintenance/${OM_ENCRYPT}${PLATFORM}/
#rm -f older/*.* || true
mv -f OpenModelica* older/ || true
ENDSSH
scp  -i $HOME/.ssh/id_rsa -o UserKnownHostsFile=$HOME/.ssh/known_hosts OpenModelica*${PLATFORM}* ${SSHUSER}@build.openmodelica.org:public_html/omc/builds/windows/releases/1.17/maintenance/${OM_ENCRYPT}${PLATFORM}/
ssh  -i $HOME/.ssh/id_rsa -o UserKnownHostsFile=$HOME/.ssh/known_hosts ${SSHUSER}@build.openmodelica.org <<ENDSSH
#commands to run on remote host
cd public_html/omc/builds/windows/releases/1.17/maintenance/${OM_ENCRYPT}${PLATFORM}/
pwd
